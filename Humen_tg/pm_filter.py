...
async def fil_mod(client, message): 
      ...
      try: 
         ...
         return await message.reply("**☹️ɪɴᴄᴏᴍᴘʟᴇᴛᴇ ᴄᴏᴍᴍᴀɴᴅ...**")
      
      m = await message.reply("**sᴇᴛᴛɪɴɢ.../**")

      if args in mode_on:
          FILTER_MODE[str(message.chat.id)] = "True" 
          await m.edit("**♨️ ᴀᴜᴛᴏғɪʟᴛᴇʀ ᴇɴᴀʙʟᴇᴅ**")
...
async def next_page(bot, query):
    ...
    if not search:
        await query.answer("Aʜʜ... Bᴜᴛᴛᴏɴ Exᴘɪʀᴇᴅ Pʟᴇᴀsᴇ Rᴇǫᴜᴇsᴛ Aɢᴀɪɴ 🙂.", show_alert=True)
        return
...
async def advantage_spoll_choker(bot, query):
    ...
    await query.answer('ᴡᴀɪᴛ ɴᴊᴀɴ ᴏɴɴ ᴛʜᴀᴘᴘᴀᴛᴛᴇ...')
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('ᴛʜɪs ᴍᴏᴠɪᴇ ɴᴏᴛ ʀᴇʟᴇᴀsᴇᴅ ᴏʀ ɴᴏᴛ ᴀᴅᴇᴅᴅ ɪɴ ᴍʏ ᴅᴀᴛᴀʙᴀsᴇ!!!,ᴘʟᴇᴀsᴇ ᴄᴏɴᴛᴀᴄᴛ ᴀᴅᴍɪɴ🙃')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [
            [
                InlineKeyboardButton('🎭 ᴍᴏᴠɪᴇ ᴄᴏʟʟᴇᴄᴛɪᴏɴ', url='https://t.me/Cinemaveed')
            ]
            ]
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton("📥  𝖣𝗈𝗐𝗇𝗅𝗈𝖺𝖽  📥", url = ms.link)
                        ],
                        [
                            InlineKeyboardButton("⚠️ 𝖢𝖺𝗇'𝗍 𝖠𝖼𝖼𝖾𝗌𝗌 ❓ 𝖢𝗅𝗂𝖼𝗄 𝖧𝖾𝗋𝖾 ", url = f"{CHANNEL_LINK}")
                        ]
                    ]
                )
            )
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer(f"𝖨 𝖫𝗂𝗄𝖾 𝖸𝗈𝗎𝗋 𝖲𝗆𝖺𝗋𝗍𝗇𝖾𝗌𝗌 𝖡𝗎𝗍 𝖣𝗈𝗇'𝗍 𝖡𝖾 𝖮𝗏𝖾𝗋 𝖲𝗆𝖺𝗋𝗍𝗇𝖾𝗌𝗌😏",show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        InlineKeyboardButton('🔰 ʙᴀᴄᴋ ᴛᴏ ᴍᴀɪɴ ᴍᴇɴᴜ 🔰', callback_data='nihu')   
    ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.START_TXT.format(query.from_user.mention, temp.U_NAME, temp.B_NAME),
        ...
        buttons = [[       
            ...
            InlineKeyboardButton('⛑️ ᴄᴏɴᴛᴀᴄᴛ ᴏᴡɴᴇʀ', url='https://t.me/nishn_ea')
        ], [
        ...
        InlineKeyboardButton('⛔️ ᴄʟᴏsᴇ ᴘᴀɢᴇs ⛔️', callback_data='close_data')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        await query.answer("ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ ғᴏʀ ᴀ ғᴇᴡ sᴇᴄᴏɴᴅ....")
        buttons = [[
            InlineKeyboardButton('ʙᴀᴄᴋ', callback_data='help'),
            InlineKeyboardButton('♻️ ʀᴇғʀᴇsʜ', callback_data='rfrsh')
        ]]
