...
ᴍʏ ɴᴀᴍᴇ ɪꜱ  <a href=https://t.me/Adv_AutoFilBot><b>『 ᴡᴀᴢᴇᴇᴍ ʙᴏᴛ 』</b></a>  ɪ ᴄᴀɴ ᴘʀᴏᴠɪᴅᴇ ʏᴏᴜ ᴍᴏᴠɪᴇꜱ ᴊᴜꜱᴛ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘ ᴀɴᴅ ꜱᴇᴇ ᴍʏ ᴘᴏᴡᴇʀ 😈</b>
"""
        buttons = [[
...
async def delete(bot, message):
    ...
    if reply and reply.media:
        msg = await message.reply("ᴅᴇʟᴇᴀᴛɪɴɢ....🗑️", quote=True)
    else:
    ...
    if result.deleted_count:
        await msg.edit('**ғɪʟᴇ ᴅᴇʟᴇᴛᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ**')
    else:
...
async def delete_all_index(bot, message):
    await message.reply_text(
        '**ᴛʜɪs ᴘʀᴏᴄᴄᴇss ᴡɪʟʟ ᴅᴇʟᴇᴛᴇ ᴀʟʟ ғɪʟᴇs ғʀᴏᴍ ʏᴏᴜʀ ᴅᴀᴛᴀʙᴀsᴇ.\nᴅᴏ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ ᴛʜɪs ᴏᴘʀᴀᴛɪᴏɴ..??**',
...
async def save_template(client, message):
    sts = await message.reply("**𝖶𝖺𝗂𝗍 𝖨 𝖺𝗆 𝖢𝗁𝖺𝖼𝗄𝗂𝗇𝗀 𝖳𝗁𝗂𝗌 𝖳𝖾𝗆𝗉𝗅𝖺𝗍𝖾**")
    userid = message.from_user.id if message.from_user else None
    if not userid:
        return await message.reply(f"You Are Anonymous Admin.Use /connect {message.chat.id} in PM")
    chat_type = message.chat.type
    ...
    await sts.edit(f"Successfully Upgraded Template For {title}\n\n{template}")
